...
export async function addNewProfessions (professions: string[]): Promise<string[]> {
    const workerData: any = await UserWorker.findByPk(id, {attributes: ["profession"]});
    const totalProfessions: any = workerData.toJSON();
    const totalNewProfessions: string[] = totalProfessions.profession;
    professions.forEach((e: string) => totalNewProfessions.includes(e) ? null : totalNewProfessions.push(e))
...
export async function addNewSkills (skills: string[]): Promise<string[]> {
    const workerData: any = await UserWorker.findByPk(id, {attributes: ["skills"]});
    const totalSkills: any = workerData.toJSON();
    const totalNewSkills: string[] = totalSkills.skills;
    skills.forEach((e: string) => totalNewSkills.includes(e) ? null : totalNewSkills.push(e))
    await UserWorker.update({
